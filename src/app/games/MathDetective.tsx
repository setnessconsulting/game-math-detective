...
export default function MathDetective() {
  ...
  const renderBriefing = () => (
    <section aria-labelledby="briefing-title" className="md-flow-section">
      <div className="md-section-kicker">Case file opened</div><h2 id="briefing-title">Briefing</h2>
      <p className="md-goal-copy">{run.intro}</p>
      <div className="md-lineup" role="group" aria-label="Suspect lineup">{scene.suspects.map((suspect) => <article className="md-suspect-mini" key={suspect.id}><span className="md-avatar" aria-hidden="true">{suspect.icon}</span><strong>{suspect.name}</strong><span>Possible suspect</span></article>)}</div>
